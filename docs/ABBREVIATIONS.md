...
**BTAL** — AGF U.S. Market Neutral Anti-Beta Fund. Market-neutral anti-beta equity candidate on the risk-off 63d RS overlay, immediately after FTLS and before BIL. Same gates as the other non-CTA names: 63d total return strictly above BIL, and last above its own 200dma, or that name is skipped (none left → BIL). Exact RS ties use preference order after the other equity diversifiers (GLD > GDX > PDBC > UUP > duration > defensives > trend > CLSE > USMV > QUAL > FTLS > BTAL); CLSE, USMV, QUAL, and FTLS still win an exact tie. Top-2 50/50 and the 50bp hysteresis apply. Not CTA (not in RISKOFF_ETF_CTA_FAMILY), not gold (not in RISKOFF_ETF_GOLD_FAMILY), and not RISKOFF_ETF_COMMODITY_BETA (PDBC). No new family. Not subject to the 21-session beat-BIL confirmation; a weak or missing 21d return does not skip BTAL. Can fill non-CTA #2 when RS #1 is DBMF/KMLM (PDBC still cannot) and when RS #1 is PDBC. If no allowed non-CTA clears, that #2 is BIL. Below its own 200dma it does not qualify. Aimed at HYG-only RISK OFF when puts are gated, so it can compete with PDBC or CTA only after it clears beat-BIL and own-200. RS re-rank still waits for the NY cash close, and the 5-session hold applies. Paper / MockBroker only.
...
**CTA** — Commodity Trading Advisor / managed-futures sleeve family on the risk-off 63d RS overlay (RISKOFF_ETF_CTA_FAMILY = DBMF, KMLM). Must also beat BIL over RISKOFF_ETF_CTA_CONFIRM_DAYS (21) or that name is skipped. When RS #1 is in this set, #2 is a non-CTA qualifier other than PDBC that clears beat-BIL and own-200 (GDX may fill that slot); if none does, #2 is BIL at 50/50. Never hold two CTA names together, and never hold PDBC with a CTA name. GDX is not in this set. PDBC is not in this set.
...
**inCashSession** — `marketSession` flag, true only from 09:30 ET until the NYSE cash close (16:00 ET, 13:00 ET on early-close days). Distinct from `cashOpen` (calendar day). Web and iOS countdowns use it to choose `Cash close in` vs `Cash open in`.
...
**marketSession** — Top-level GET `/api/status` field: NYSE cash calendar in ET (`cashOpen`, `inCashSession`, `closedReason`, `holidayName`, `asOfEt`, `nextOpenEt`, `nextCloseEt`, `nextOpenAt`, `nextCloseAt`). Not a GateMode. Web header, `/m`, and the iOS essentials header count down to `nextCloseAt` while `inCashSession` (`Cash close in 2h 14m 03s`) and to `nextOpenAt` otherwise (`Cash open in …`). Tick is client-side (Swift `TimelineView` on iOS, about once a second, no extra status poll). Early close is 13:00 ET.
...
**QUAL** — iShares MSCI USA Quality Factor ETF. Quality-factor equity candidate on the risk-off 63d RS overlay, immediately after USMV and before FTLS. Same gates as the other non-CTA names: 63d total return strictly above BIL, and last above its own 200dma, or that name is skipped (none left → BIL). Exact RS ties use preference order after USMV and before FTLS (GLD > GDX > PDBC > UUP > duration > defensives > trend > CLSE > USMV > QUAL > FTLS > BTAL). Top-2 50/50 and the 50bp hysteresis apply. Not CTA (not in RISKOFF_ETF_CTA_FAMILY) and not gold (not in RISKOFF_ETF_GOLD_FAMILY); not subject to the 21-session beat-BIL confirmation. Can fill non-CTA #2 when RS #1 is DBMF/KMLM. Aimed at HYG-only RISK OFF when puts are gated. Paper / MockBroker only.
...
**RISKOFF_ETF_COMMODITY_BETA** — PDBC. Broad commodity beta on the risk-off 63d RS overlay, mutually exclusive with RISKOFF_ETF_CTA_FAMILY for the #2 diversifier only (same spirit as never-dual-CTA / never-dual-gold). Not a second ticker family and not a member of the CTA family. BTAL is not this symbol. No 21-session beat-BIL confirmation. When RS #1 is PDBC, #2 is the highest non-CTA qualifier, else BIL at 50/50. When RS #1 is DBMF or KMLM, #2 skips PDBC (next non-PDBC qualifier, else BIL). Never PDBC+DBMF or PDBC+KMLM. A lone PDBC still takes the full overlay fraction. The cash-close rebalance breaks a forbidden pair even inside RISKOFF_ETF_MIN_HOLD_SESSIONS. Paper / MockBroker only.
...
**RS** — Relative strength. Momentum score vs SPY; risk-off ETF overlay is 63-session total return of GLD/GDX/PDBC/UUP/TLT/IEF/XLU/XLP/DBMF/KMLM/CLSE/USMV/QUAL/FTLS/BTAL vs BIL, sized at RISKOFF_ETF_NOTIONAL_FRAC (40%, SPY below 200) or RISKOFF_ETF_NOTIONAL_FRAC_PUT_GATED (60%, SPY above 200 / puts gated) and split top-2 50/50 among qualifiers (beat BIL and above own 200). Mild 50bp hysteresis (RISKOFF_ETF_RS_HYSTERESIS) only when the 63d universe is complete — missing bars debounce (RISKOFF_ETF_MISSING_BARS_MAX_MISSES) short-circuits first. Re-rank and resize once per NY cash session at the cash close, not on midday bars. A name stays at least RISKOFF_ETF_MIN_HOLD_SESSIONS (5) cash sessions before an RS rotate. CTA family {DBMF, KMLM} must also beat BIL over 21 sessions (RISKOFF_ETF_CTA_CONFIRM_DAYS; missing 21d skips that CTA only). When #1 is CTA, #2 is a non-CTA qualifier other than PDBC (GDX, CLSE, USMV, QUAL, FTLS, and BTAL can fill that slot) or else BIL — never both CTAs, and never PDBC with that CTA. When #1 is PDBC, #2 is a non-CTA qualifier or else BIL. Gold family {GLD, GDX} is the same shape: when #1 is gold, #2 is a non-gold qualifier or else BIL — never GLD+GDX. That dual-gold break, and the PDBC↔CTA break, also win at the cash-close rebalance inside the 5-session hold. GDX is not 21d-gated. Gated TLT/IEF duration is not an RS pick.
...
**SYMBOL_DESCRIPTIONS** — Shared map in `shared/symbolDescriptions.ts` of Event Gate ticker → short full name (methodology ETFs/futures roots from this glossary: SPY, QQQ, HYG, GLD, GDX, PDBC, UUP, BIL, TLT, IEF, XLU, XLP, DBMF, KMLM, CLSE, USMV, QUAL, FTLS, BTAL, LQD, JNK, SJB, ACWI, IWM, and gated roots MES/MNQ/ES/NQ/ZN/ZF/ZT/ZB/SR3/6E/M6E). Web paper UI sets HTML `title` (and an accessibility label) on primary symbol labels: quote strips, sleeve instrument chips, positions, orders, blotter, overlay names, options underlyings, and gated-root chips. Dated futures (`MESU6`) and Yahoo `=F` resolve to the root; option packages resolve to the underlying. Unknown symbols show the bare ticker with no tooltip. Display only. Does not change MockBroker orders. iOS essentials does not list individual tickers, so it has no VoiceOver symbol description.
