...
**RISKOFF_ETF_COMMODITY_BETA** — PDBC. Broad commodity beta on the risk-off 63d RS overlay, mutually exclusive with RISKOFF_ETF_CTA_FAMILY for the #2 diversifier only (same spirit as never-dual-CTA / never-dual-gold). Not a second ticker family and not a member of the CTA family. No 21-session beat-BIL confirmation. When RS #1 is PDBC, #2 is the highest non-CTA qualifier, else BIL at 50/50. When RS #1 is DBMF or KMLM, #2 skips PDBC (next non-PDBC qualifier, else BIL). Never PDBC+DBMF or PDBC+KMLM. A lone PDBC still takes the full overlay fraction. The cash-close rebalance breaks a forbidden pair even inside RISKOFF_ETF_MIN_HOLD_SESSIONS. Paper / MockBroker only.
...
**SYMBOL_DESCRIPTIONS** — Shared map in `shared/symbolDescriptions.ts` of Event Gate ticker → short full name (methodology ETFs/futures roots from this glossary: SPY, QQQ, HYG, GLD, GDX, PDBC, UUP, BIL, TLT, IEF, XLU, XLP, DBMF, KMLM, CLSE, USMV, QUAL, FTLS, LQD, JNK, SJB, ACWI, IWM, and gated roots MES/MNQ/ES/NQ/ZN/ZF/ZT/ZB/SR3/6E/M6E). Web paper UI sets HTML `title` (and an accessibility label) on primary symbol labels: quote strips, sleeve instrument chips, positions, orders, blotter, overlay names, options underlyings, and gated-root chips. Dated futures (`MESU6`) and Yahoo `=F` resolve to the root; option packages resolve to the underlying. Unknown symbols show the bare ticker with no tooltip. Display only. Does not change MockBroker orders. iOS essentials does not list individual tickers, so it has no VoiceOver symbol description.
