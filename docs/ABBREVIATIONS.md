...
**GLD** — SPDR Gold Shares. First-preference candidate on the risk-off 63d RS overlay vs BIL. Broad commodity beta (PDBC) is next on an exact tie. Still needs to be above its own 200dma after the RS pick; otherwise the overlay parks in BIL.
...
**RISKOFF_ETF_CTA_FAMILY** — Managed-futures / CTA tickers on the risk-off 63d RS overlay (DBMF, KMLM). Each member must also beat BIL on RISKOFF_ETF_CTA_CONFIRM_DAYS or it is skipped. When RS #1 is in this set, #2 is a non-CTA qualifier that clears beat-BIL and own-200; if none does, #2 is BIL at 50/50. Never KMLM+DBMF. Add future CTA names here and to RISKOFF_ETF_SYMBOLS. PDBC, CLSE, USMV, and FTLS are not members.
