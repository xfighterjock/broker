...
**EVENT_GATE_OPS_TOKEN** — Optional long-lived HTTPS ops bearer (VPS `/opt/broker/.env`, never git). When set, `Authorization: Bearer` matching the env value authenticates a narrow freeze/status scope (GET /api/status, GET/PUT /api/freeze, GET /api/health, GET /api/sleeves). Not a users-table session and not GATE ON/OFF. When unset, behavior unchanged. Agents freeze-save and status-check at https://broker.logikmancer.com without the Mac.
