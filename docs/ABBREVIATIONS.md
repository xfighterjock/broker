...
**CTA** — Commodity Trading Advisor / managed-futures sleeve family on the risk-off 63d RS overlay (RISKOFF_ETF_CTA_FAMILY = DBMF, KMLM). Must also beat BIL over RISKOFF_ETF_CTA_CONFIRM_DAYS (21) or that name is skipped. When RS #1 is in this set, #2 prefers a non-CTA qualifier.
...
**ETF** — Exchange-traded fund. Risk-off 63d RS overlay is GLD/UUP/TLT/IEF/XLU/XLP/DBMF/KMLM/CLSE/USMV/BIL sized at RISKOFF_ETF_NOTIONAL_FRAC (40% of the $100k book) when SPY is below 200, or RISKOFF_ETF_NOTIONAL_FRAC_PUT_GATED (60%) while SPY is above 200 and puts stay gated; split 50/50 across the top-2 qualifiers (beat BIL and above own 200). CTA family {DBMF, KMLM} must also beat BIL over 21 sessions. USMV is in the basket and is not CTA. Missing overlay bars debounce via RISKOFF_ETF_MISSING_BARS_MAX_MISSES (hold last sleeve; do not flatten on a single miss). A missing 21d CTA check skips that CTA and does not start that debounce. Gated duration is a separate TLT/IEF long at RISKOFF_DURATION_NOTIONAL_FRAC (20%). Gate names are SPY/ACWI/HYG/UUP.
...
**RISKOFF_ETF_CTA_FAMILY** — Managed-futures / CTA tickers on the risk-off 63d RS overlay (DBMF, KMLM). Each member must also beat BIL on RISKOFF_ETF_CTA_CONFIRM_DAYS or it is skipped. When RS #1 is in this set, #2 prefers a non-CTA qualifier; the other CTA is #2 only if no non-CTA qualifier exists and that other CTA passed the 21d check. Add future CTA names here and to RISKOFF_ETF_SYMBOLS. CLSE and USMV are not members.
