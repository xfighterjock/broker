...
- SPA cookie `eg.sid` lives in Redis (`eg:sess:` prefix via connect-redis). Cookie `maxAge` is 7 days (`7 * 24 * 3600 * 1000` in `buildSessionMiddleware`). Cookie name and Redis prefix differ on purpose. `COOKIE_SECURE=1` or `NODE_ENV=production` sets `cookieSecure` in `server/src/config.ts` (`env.COOKIE_SECURE === "1" || nodeEnv === "production"`), which express-session uses as `cookie.secure`.
- iOS opaque bearer lives in Postgres `user_sessions` (sha256 of the token). `SESSION_TTL_MS` is 30 days.
- Secure cookie behind nginx (`server/src/index.ts`): before session middleware, the Express root does `root.set("trust proxy", 1)` because nginx terminates TLS and sets `X-Forwarded-Proto`. Without that hop, `req.secure` stayed false on the HTTP bind (`127.0.0.1:3001`) and express-session skipped `Set-Cookie` when `cookie.secure` was true. Login still returned 200 + bearer; the SPA (`credentials: include` in `client/src/api.ts`) never got `eg.sid`. Trust proxy is what makes Secure cookies actually emit. Covered by `tests/users-auth.test.ts`: `cookie.secure=true` + `X-Forwarded-Proto: https` must emit `eg.sid` with Secure.

- POST `/api/auth/login` `{ username, password }` (AUTH_MODE=users) returns `{ ok, username, token, expiresAt }` and should set cookie `eg.sid` for the SPA (`credentials: include`). iOS stores the bearer in the Keychain and sends `Authorization: Bearer`.
