...
Quote strip (RISKOFF_QUOTE_STRIP): SPY, QQQ, HYG, GLD, UUP, BIL, TLT, IEF, XLU, XLP, DBMF, KMLM, CLSE, USMV, LQD, JNK, SJB. SJB is visibility only (not a traded inverse). symbolsForSleeve always includes this strip for riskoff, then any extra card instruments. Massive delayed quotes/dailies score KMLM, CLSE, and USMV like the other overlay names. CLSE and USMV are not CTA (do not add them to RISKOFF_ETF_CTA_FAMILY); either can fill non-CTA #2 when RS #1 is DBMF or KMLM. USMV is not subject to the 21-session beat-BIL confirmation.
