...
- POST /api/paper/reset `{ sleeveId }`: MockBroker only. Wipes one sleeve to a clean $100k book without a delayed last or fill: drop that sleeve's mock positions, cancel its working orders, zero paper journal + blotter, align the session mark so daily and total P/L are 0. Other sleeves, AUTO PAPER, and GATE stay as-is. Refuses when the broker is not MockBroker. Does not place a live or E*TRADE order. POST /api/paper/close still needs a delayed last (or vertical/overlay mark) and is the wrong ops path before cash prints. POST /api/flatten still only flattens gated day-sleeve names.
