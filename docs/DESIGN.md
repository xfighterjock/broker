...
- Client: Vite + React SPA (client/). Desktop layout; phones (narrow viewport or /m) get essentials (clock, US cash closed/holiday strip, GATE, RISK, AUTO PAPER + D/M/O/Ow/R chips, Flatten, sleeve P/L). Native iOS Event Gate (ios/, bundle com.logikmancer.mybroker) is the phone Event Gate client: same essentials plus FCM and a paged Activity screen. Web /m remains for browsers. No WKWebView wrapper. Cold start paints login or last-session essentials immediately from Keychain + a cached GET `/api/status` snapshot — it does not wait on Firebase/FCM or a live status call. `FirebaseApp.configure()`, APNs, and FCM attach run after the first frame off the MainActor (`AppDelegate.startFirebaseAfterFirstFrame` via `Task.detached`) so they cannot stall the first GET `/api/status`. While unlocked, iOS polls GET `/api/status` every 3 seconds (`StatusController.pollInterval`) in the background. BrokerAPI uses ephemeral `URLSession`s (not `URLSession.shared`) with `waitsForConnectivity = false`. `timeoutInterval` does not abort a TCP/TLS connect that never completes, so a racing Task cancels the `URLSessionTask` after 8s and retries once on a fresh session. A blackholed first GET `/api/status` must fail with a visible error, not wait for a wifi/cellular path-update. A swipe-kill is a new process, not a leftover socket. That snapshot does not wait on delayed-quote paper marks (see Paper engine). Activity history is not on that snapshot — the phone loads GET `/api/activity` when the Activity screen opens (newest page first, older pages on scroll) and must not block first paint.
