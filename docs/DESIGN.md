...
- Client: Vite + React SPA (client/). Desktop layout; phones (narrow viewport or /m) get essentials (clock, US cash closed/holiday strip, GATE, RISK, AUTO PAPER + D/M/O/Ow/R chips, Flatten, sleeve P/L). Native iOS Event Gate (ios/, bundle com.logikmancer.mybroker) is the phone Event Gate client: same essentials plus FCM and a paged Activity screen. Web /m remains for browsers. No WKWebView wrapper. While unlocked, iOS polls GET /api/status every 3 seconds (`StatusController.pollInterval`). BrokerAPI uses a dedicated `URLSession` (not `URLSession.shared`) with a 10s request timeout, `waitsForConnectivity = false`, and one automatic retry on transport timeout so a dead cellular/wifi path cannot hang login or the first status forever — fail or retry fast and surface the error. That snapshot does not wait on delayed-quote paper marks (see Paper engine). Activity history is not on that snapshot — the phone loads GET `/api/activity` when the Activity screen opens (newest page first, older pages on scroll) and must not block first paint.
...
- Native iOS client (`ios/EventGate`, display name Event Gate, bundle `com.logikmancer.mybroker`):
  ...
  2) Login: username/password against `/api/auth/login`. Bearer in Keychain. Home screen is essentials (clock/mode, US cash closed/holiday strip from `marketSession`, GATE, RISK, AUTO PAPER + D/M/O/Ow/R, Flatten with `FLATTEN_CONFIRM`, sleeve P/L, E*TRADE PIN). Toolbar Activity opens `ActivityLogView`: GET `/api/activity` newest-first, older pages on scroll (`loadMore` at the last row). While unlocked, `StatusController` polls GET `/api/status` every 3 seconds (`pollInterval`) and stops when locked — that poll does not fetch the activity journal and is non-blocking for paper-stop marks (status kicks `markPaperQuiet`; GET `/api/quotes` still awaits). `BrokerAPI` injects `BrokerHTTPPerforming` (default: dedicated ephemeral `URLSession` from `BrokerTransport`, 10s request / 15s resource timeout, `waitsForConnectivity` false, one retry on `URLError.timedOut`). First status/login must not sit on a dead interface until wifi/cellular flips. Settings (secondary) holds base URL, biometric unlock, and FCM Register/Revoke/Test. After token + session, `POST /api/notifications/tokens/register` with `Authorization: Bearer` and `{ platform: "ios", token, deviceLabel? }`. App still sends `x-remote-user: event-gate` so tokens match the VPS default principal.
