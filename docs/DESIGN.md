...
Quote strip (RISKOFF_QUOTE_STRIP): SPY, QQQ, HYG, GLD, UUP, BIL, TLT, IEF, XLU, XLP, DBMF, KMLM, CLSE, LQD, JNK, SJB. SJB is visibility only (not a traded inverse). symbolsForSleeve always includes this strip for riskoff, then any extra card instruments. Massive delayed quotes/dailies score KMLM and CLSE like the other overlay names. CLSE is not CTA (do not add it to RISKOFF_ETF_CTA_FAMILY); it still qualifies as a non-CTA #2 when RS #1 is DBMF or KMLM.
