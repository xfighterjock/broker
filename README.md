...
Option expiries and chains are live E*TRADE market data (`https://api.etrade.com/v1/`). Paper fills stay MockBroker. Never place E*TRADE orders. OAuth access tokens die overnight ET; refresh with `node scripts/etrade-oauth.mjs request --env production` then `node scripts/etrade-oauth.mjs access <PIN> --env production`. Keys live in gitignored `.env.etrade` (`ETRADE_ENV=production`, `ETRADE_PROD_KEY`/`SECRET`, `ETRADE_PROD_ACCESS_TOKEN`/`SECRET`). Never paste keys.
