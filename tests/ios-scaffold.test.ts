...
describe("iOS Event Gate scaffold", () => {
  ...
  it("home screen is essentials and Settings is secondary; no WKWebView shell", () => {
    ...
    expect(essentials).toContain("CashCountdownText");
    expect(essentials).toContain("cash-cd");
    expect(essentials).toContain("PeriodicTimelineSchedule");
    expect(format).toContain("cashCountdownTarget");
    expect(format).toContain("formatCashCountdown");
    expect(format).toContain("Cash open in");
    expect(format).toContain("Cash close in");
    expect(format).toContain("inCashSession");
    expect(format).toContain("nextOpenAt");
    expect(format).toContain("nextCloseAt");
    const design = readFileSync(resolve("docs/DESIGN.md"), "utf8");
    expect(design).toContain("iOS Event Gate essentials header");
    expect(design).not.toContain("iOS still shows the closed strip only");
